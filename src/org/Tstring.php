...
class Tstring
{
    ...
    // 返回唯一的uuid
    public static function uuid_md5()
    {
        return md5(self::uuid());
    }
    ...
    //去br
    public static function nobr($str)
    {
        $rep = array('<br>', '<br />', '<br/>');

        return strval(str_ireplace($rep, "\r\n", $str));
    }

    //去url中的参数字段
    public function filter_url($url, $filters = array('page'))
    {
        $parse = parse_url($url);
    ...
    // ip格式
    public static function ipmatch($ip)
    {
    ...
    // 偏移纠正量
    public static function modify_id($i)
    {
        $key = round($i * 100 / 46, 4) * 10000;
        if ($key > 99999) {
            $key = substr($key, -5);
            $key = intval($key);
        }
